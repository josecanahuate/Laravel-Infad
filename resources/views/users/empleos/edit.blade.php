        ...
        <form method="POST" action="{{ route('users.empleos.update', $experienciaLaboral) }}">
            ...
            <div class="row">
                ...
                <div class="form-group col-lg-4 col-md-4 mb-3" data-select2-id="29">
                    ...
                    <select class="form-control select2 select2-hidden-accessible" id="sector_empresa" name="sector_empresa"
                        ...
                        @foreach (['Tecnología', 'Salud', 'Educación', 'Finanzas', 'Manufactura', 'Comercio minorista', 'Agricultura', 'Construcción', 'Energía', 'Medios de comunicación', 'Servicios profesionales', 'Bienes raíces', 'Transporte', 'Hotelería y turismo', 'Entretenimiento', 'Consultoría', 'ONGs y organizaciones sin fines de lucro', 'Gobierno y sector público', 'Investigación y desarrollo (I+D)', 'Otros'] as $sectores)
                            <option value="{{ $sectores }}"
                                {{ $experienciaLaboral->sector_empresa == $sectores ? 'selected' : '' }}>{{ $sectores }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="form-group col-lg-3 col-md-3 mb-3" data-select2-id="29">
                    ...
                    <select class="form-control select2 select2-hidden-accessible" id="estatus_empleo" name="estatus_empleo"
                        style="width: 100%;" data-select2-id="1" tabindex="-1" aria-hidden="true"
                        value="{{ $experienciaLaboral->estatus_empleo }}">
                        @foreach (['Concluido', 'Actualmente laborando'] as $estatus)
                            <option value="{{ $estatus }}"
                                {{ $experienciaLaboral->estatus_empleo == $estatus ? 'selected' : '' }}>
                                {{ $estatus }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-lg-3 col-md-3 mb-3">
                    ...
                    <input type="date" class="form-control datetimepicker-input" data-target-input="nearest"
                        value="{{ $experienciaLaboral->fecha_inicio }}" name="fecha_inicio" id="fecha_inicio">
                    @error('fecha_inicio')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-3 col-md-3 mb-3">
                    ...
                    <input type="date" class="form-control datetimepicker-input" data-target-input="nearest"
                        value="{{ $experienciaLaboral->fecha_fin }}" name="fecha_fin" id="fecha_inicio">
                    @error('fecha_fin')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
